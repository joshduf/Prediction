...
# Loads positive and negative examples
def loadXYDedupe(PATHS, ID_INDEX, TEXT_INDEX, Y_INDEX, NUM_POS, NUM_NEG, PROB_POS, PROB_NEG, MAX_FIELD, TRAIN_SPLIT, dedupe_dict):
    csv.field_size_limit(sys.maxsize)
...
#                    continue
                ...
                if val == 0:
                    dedupe_neg.add(dedupe_id)
                else:
                    dedupe_pos.add(dedupe_id)

                if neg >= NUM_NEG and pos >= NUM_POS:
                    break
                
                if val == 0 and random.random() > PROB_NEG:
                    continue
                    
                if val == 1 and random.random() > PROB_POS:
                    continue

                if val == 0 and neg >= NUM_NEG:
                    continue

                if val == 1 and pos >= NUM_POS:
                    continue
    ...
    print("Unique positives: " + str(len(dedupe_pos)))
    print("Unique negatives: " + str(len(dedupe_neg)))
    print("Total processed: " + str(total_processed))
...
def readDataDedupe(PATHS, ID_INDEX, TEXT_INDEX, Y_INDEX, TRAIN_SPLIT, NUM_POS, NUM_NEG, PROB_POS, PROB_NEG, MAX_FIELD,
                   dedupe_dict):
    x_train, y_train, x_valid, y_valid, x_test, y_test, pos, neg = loadXYDedupe(PATHS, ID_INDEX, TEXT_INDEX, Y_INDEX,
                                                                                NUM_POS, NUM_NEG, PROB_POS, PROB_NEG,
                                                                                MAX_FIELD, TRAIN_SPLIT, dedupe_dict)
